...
Rounds 1–2 (`docs/reports/round1-pipeline.md`, `docs/reports/round2-results.md`) built the pipeline and established the temporal-input lesson.
...
edge-class hardware). Training uses `data/videos/07_05.mp4` only; `data/videos/10_06.mp4` is a pure test video
...
| sub-pixel trail placement (fractional per-channel offsets baked in via warpAffine) | ~1 px/frame drifter trails quantized away |
...
- **Read the first two rows together.** *Coast smoothing* is an **offline** post-process — it
  interpolates coasted positions between real-detection anchors on both sides, so it uses up to 60
  frames of look-ahead and is only legal for a recorded-and-reviewed surveillance product. The
  causal, frame-by-frame number is the second row: **0.995 val, 0.998 full, 1.000 test**. On the
  unseen 10_06 test video the two modes agree at 1.000, so the headline "1.000 on the video it was
  never trained on" needs no look-ahead; on 07_05 it does, and every 1.000 quoted for PC-MAX on
  07_05 anywhere else means the smoothed row.
...
| rt-c v2 (round-2 nano, per-frame) | 0.679 / 0.658 | 0.894 / 0.894 | 10.4 | 78.1 |
...
[`realtime/work/eval3_0705_val.md`](../../realtime/work/eval3_0705_val.md) /
[`eval3_1006_test.md`](../../realtime/work/eval3_1006_test.md) — with the per-frame detections and
tracks they were scored from under `work/det3/` and `work/tracks3/` (the coast-smoothing rows come
from `work/det3/*/tracked-pcmax-smooth.json`, committed alongside). The training runs themselves are
gitignored build output and were not kept. With no honest
...
confirmed within 7 frames. Read that 07_05 1.000 as the *smoothed* row of §5, not the causal
one: `run_final.py` writes `tracked_dets.json` through `tools/tracks_to_dets.py --smooth-coast`,
so the shipped package's headline tracked metric is the offline surveillance-mode number. On the
unseen 10_06 the two modes agree at 1.000, so nothing in the test column depends on the choice.
...
`final/edge_rt/edge_n1280.pt` plus the portable `edge_n1280.onnx` / `edge_n1280.fp16.onnx`.
**No TensorRT engine is distributed** — engines are architecture-specific, so the 74 fps row above
is what EDGE-RT does once you have built one on your own device
(`realtime/tools/export_models.py`); `run_final.py` otherwise falls back to the `.pt`, which is
correct but not TRT-fast. Usage, retraining recipe and alarm semantics:
