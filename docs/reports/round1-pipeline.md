...
> **Partly superseded by [round 2](round2-results.md) — read that first if you only read one.**
> The semi-automatic ground truth used throughout this report follows a **bird**, not the drone,
> from frame 336 to the end. Every number below that involves the late "cruise across the sky" is
> therefore scored against the wrong object: re-scored against the corrected hand labels, the
> round-1 champion `moe-hybrid` scores AP(far) **0.004** on the val segment, not 0.401
> (`work/eval_user_val.md`, reproducible with
> `python -m dronedet eval --gt work/gt_user.json --dets work/det/moe-hybrid.json --frames 342:548`).
> What survives, and is why this report is kept: the pipeline shape, the resolution/SAHI/motion
> comparisons over frames 0–290 where the two ground truths agree to 1.2 px median, and the
> tiny-object training saga — all three carried straight into round 2 and are still load-bearing.
...
In git (open them without re-running anything):

- [`work/eval_val.md`](../../work/eval_val.md), [`work/eval_full.md`](../../work/eval_full.md) — the comparison tables above
- [`work/gt.json`](../../work/gt.json), `work/gt_verify/*.png` — the semi-automatic ground truth + its verification sheets (superseded by `work/gt_user.json`; see the banner at the top)
...
- `work/det/*.json`, `work/tracks/*.json` — per-method detections and tracker outputs; score with `tools/eval_tracks.py`

Regenerable, and therefore gitignored: the annotated videos (`work/vis/`, rebuilt by
`tools/final_round.py`).
