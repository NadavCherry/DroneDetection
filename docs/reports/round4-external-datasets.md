...
Neither dataset is redistributed here. Both are third-party releases under their own licences
(above), obtained from the sources in the Reference column and unpacked locally into `data/external/`,
which `.gitignore` excludes along with the converted YOLO layouts under `work/ext_datasets/`. So a
fresh clone has neither the raw frames nor the tiles, and the reproduce block in §7 starts from a
download you have to do yourself.
...
**In-domain, but not free — ARD-MAV phantom05 (unseen clip, AP 0.47):** the same model, a harder
clip. It holds the drone at f659 and f1139 and produces nothing at f179, where the target is a
handful of pixels against a facade, or at f1619 over foliage. In-domain does not mean solved
— the 0.763 corpus AP above is an average over clips like both of these.
![ARD-MAV hard](../media/external/panel_ard_hard.png)
...
**How tiny the target is** (full frame + zoom inset), on ARD-MAV and on this repo's own 10_06:
![context](../media/external/context_ardmav.png)
![context 10_06](../media/external/context_1006.png)

The tracking result on an unseen ARD-MAV clip is tabulated in §6 (phantom09: coverage 0.975 at
1.09 px). Its annotated video was not committed; regenerate one with `python -m dronedet render`
after the detect/track commands in §7.
...
Everything these commands read and write lives under `data/external/` and `work/ext_datasets/`,
both gitignored, and the trained checkpoint (`work/runs/ardmav-tiled-640/`) is gitignored too — the
corpus-level numbers in §2–§5 above are the record that was kept; the per-video breakdown behind
them was not. Re-running `tools/eval_external.py` on a rebuilt corpus reproduces it.
