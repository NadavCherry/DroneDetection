...
def main() -> None:
    ...
    # Fine-tuning a checkpoint that already knows real drones is not the same
    # job as training one from COCO. At the default 0.01 the first epochs walk
    # far enough from the pretrained weights to forget what the large real
    # dataset paid for -- which here is precisely the long-range sensitivity
    # that made the model worth fine-tuning. Left as None so every existing
    # invocation keeps ultralytics' own default.
    ap.add_argument("--lr0", type=float, default=None,
                    help="initial LR; lower (~0.002) when fine-tuning")
    ap.add_argument("--freeze", type=int, default=None,
                    help="freeze the first N layers (backbone preservation)")
    a = ap.parse_args()
    ...
    model.train(
        ...
        workers=4,
        plots=True,
        **({"lr0": a.lr0} if a.lr0 is not None else {}),
        **({"freeze": a.freeze} if a.freeze is not None else {}),
    )
