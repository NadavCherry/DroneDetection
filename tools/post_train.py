...
``RUN_DIR`` is where Ultralytics actually wrote the run, which is *not* under
this repository: ``~/.config/Ultralytics/settings.json`` redirects ``runs_dir``
and nests relative ``project=`` paths under it (see docs/guides/retrain.md).
Point ``$DRONEDET_RUN_DIR`` at the run you want to promote.
"""
...
PY = ".venv/bin/python"
RUN_DIR = Path(os.environ.get("DRONEDET_RUN_DIR", "work/runs/ft-p2-1280"))
